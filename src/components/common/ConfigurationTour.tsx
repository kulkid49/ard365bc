...
const STORAGE_KEY = 'qa.configurationTour.completed'
...
export function ConfigurationTour({
  tab,
  setTab,
}: {
  tab: 'integrations' | 'agents' | 'hitl' | 'templates' | 'workflow' | 'security' | 'reports'
  setTab: (v: 'integrations' | 'agents' | 'hitl' | 'templates' | 'workflow' | 'security' | 'reports') => void
}) {
  const [searchParams, setSearchParams] = useSearchParams()
  const forceStart = searchParams.get('tour') === 'config'
        ...
        subSteps: [
          {
            ...
            body:
              'Welcome to Configuration & Administration — the command center where you define how the entire Agentic AI AR system behaves. All settings that control automation, integrations, and rules are managed here.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            ...
            title: 'Left Navigation Menu / Tabs',
            body:
              'This control plane organizes all configuration areas. Jump between Integrations, AI rules, HITL settings, templates, and security policies with a clear, governed workflow.',
          },
        ],
      },
      {
        id: 'integrations',
        ensure: { tab: 'integrations' },
        subSteps: [
          {
            id: 'integrations',
            selector: '[data-tour="config-d365"]',
            title: 'Dynamics 365 Business Central',
            body:
              'This is the most critical integration. Configure the OData connection to Dynamics 365 Business Central, authentication, endpoints, and deep links so agents can create and update records reliably.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'd365-test',
            selector: '[data-tour="config-d365-test"]',
            title: 'D365 BC Connection Status & Test',
            body:
              'Live connection health with one-click testing validates endpoints, authentication, and permissions. This ensures autonomous agents can post Sales Orders and invoices without breaking the flow.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'irp',
            selector: '[data-tour="config-irp"]',
            title: 'GST E-Invoice Portal (IRP) Settings',
            body:
              'Configure IRP connectivity and verify IRN generation. This is the foundation for automated GST e-invoicing with signed QR code compliance.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'agents',
            selector: '[data-tour="config-agents"]',
            title: 'AI Agents & Rules Engine',
            body:
              'Define confidence thresholds, retry logic, and risk flag definitions. These rules decide when the system stays zero-touch and when it must route to HITL for governance.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'hitl',
            selector: '[data-tour="config-hitl"]',
            title: 'Human-in-the-Loop & Escalation Matrix',
            body:
              'Edit HITL checkpoints and SLA timers, then define escalation paths for approvals and exceptions. This ensures no transaction waits beyond acceptable limits.',
          },
        ],
      },
      {
        id: 'templates',
        ensure: { tab: 'templates' },
        subSteps: [
          {
            id: 'templates',
            selector: '[data-tour="config-templates"]',
            title: 'Email & Notification Templates',
            body:
              'Manage the templates used for HITL alerts, approval requests, and invoice dispatch. Standardized templates keep communication consistent and compliant.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'security',
            selector: '[data-tour="config-security"]',
            title: 'Security & Compliance Settings',
            body:
              'Role-based access control, audit log retention, and encryption policies ensure the system meets statutory and internal governance requirements while remaining operationally fast.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'validation',
            selector: '[data-tour="config-validation"]',
            title: 'Live Validation',
            body:
              'Live validation summarizes impact before changes go live. This reduces risk by showing policy health and integration status prior to enabling high-autonomy modes.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            ...
            title: 'Policy Checks',
            body:
              'Built-in governance checks validate RBAC, audit retention, template variables, and safe threshold ranges. This makes the Agentic system configurable while staying controlled and audit-ready.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            ...
            body:
              'Configuration is the control plane behind autonomy. With integrations healthy and rules defined, the agents can run zero-touch for standard transactions and route only the right exceptions to HITL.',
          },
        ],
      },
    ],
        ...
        <div className="fixed inset-0 z-[122] grid place-items-center">
          ...
          <Card className="relative w-[min(92vw,760px)] rounded-3xl p-6">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-2xl font-semibold text-slate-900 dark:text-slate-50">You’re in Control</div>
                <div className="mt-2 text-sm text-slate-600 dark:text-slate-400">
                  You now understand how to configure integrations, rules, HITL triggers, and security controls that govern autonomous execution. With strong governance, Agentic AI stays fast while remaining
                  safe, compliant, and audit-ready.
                </div>
              </div>
