...
const STORAGE_KEY = 'qa.hitlTour.completed'
...
export function HitlWorkbenchTour() {
  const [searchParams, setSearchParams] = useSearchParams()
  const forceStart = searchParams.get('tour') === 'hitl'
    ...
    () => [
      {
        ...
        subSteps: [
          {
            id: 'page-header',
            selector: '[data-tour="hitl-header"]',
            title: 'HITL Workbench – Extraction Review',
            body:
              'Welcome to the HITL Workbench. This page activates automatically when the Contract Intelligence Agent’s confidence is below 92%. It is the primary Human-in-the-Loop interface for reviewing AI-extracted contract data.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'confidence',
            selector: '[data-tour="hitl-confidence"]',
            title: 'AI Confidence Badge',
            body:
              'The AI confidence score is displayed prominently. Anything below 92% routes the case into HITL so humans can verify accuracy before the transaction moves forward in the Agentic pipeline.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'split',
            selector: '[data-tour="hitl-split"]',
            title: 'Split-Screen Review Workspace',
            body:
              'The screen is split into two parts: the document viewer and the editable extraction form. This side-by-side design makes review fast, accurate, and auditable — without losing context.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'pdf',
            selector: '[data-tour="hitl-pdf"]',
            title: 'Document PDF Viewer (with AI Highlighting)',
            body:
              'This viewer keeps the original contract visible while you review. AI highlights help you locate customer details, billing terms, and tax clauses quickly, so HITL decisions are consistent and fast.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'form',
            selector: '[data-tour="hitl-form"]',
            title: 'Editable Extraction Form',
            body:
              'This form is pre-filled by the Contract Intelligence Agent. Each section represents structured outputs (customer, billing, tax, risk) that downstream agents will use to sync into D365 BC and continue the case automatically.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'field-customer',
            selector: '[data-tour="hitl-field-customer"]',
            title: 'Field-Level Controls (Example)',
            body:
              'Each field can be edited during HITL. Confidence badges and “Use AI Value” shortcuts speed up review, while edits create feedback signals to continuously improve future extractions.',
          },
          {
            id: 'field-gst',
            selector: '[data-tour="hitl-field-gst"]',
            title: 'Field-Level Controls (Tax Example)',
            body:
              'Tax fields are high-impact for compliance. HITL confirmation here protects GST correctness, reduces rework later, and ensures downstream agents can proceed zero-touch with confidence.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'feedback',
            selector: '[data-tour="hitl-feedback"]',
            title: 'AI Feedback Panel',
            body:
              'After review, you can provide direct feedback about what the model got wrong. This closes the continuous learning loop so accuracy increases over time across all future cases.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'actions',
            selector: '[data-tour="hitl-actions"]',
            title: 'Action Bar',
            body:
              'Save a draft, re-run extraction, escalate, or submit to continue. “Submit & Continue” pushes corrected data to the next agents (Customer Master → Billing Agent) and resumes the Agentic flow automatically.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'd365-validate',
            selector: '[data-tour="hitl-d365-validate"]',
            title: 'Validate Customer in D365 BC',
            body:
              'This performs a real-time check against Dynamics 365 Business Central to detect duplicates or mismatches before creating or updating records. It prevents downstream errors and keeps ERP data clean.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'progress',
            selector: '[data-tour="hitl-progress"]',
            title: 'Progress Summary & Mandatory Fields',
            body:
              'The system tracks extracted fields and highlights remaining low-confidence mandatory items. This ensures nothing is missed before the case proceeds — improving accuracy while keeping turnaround fast.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            ...
            body:
              'You’re ready to complete HITL reviews confidently. Human expertise + Agentic AI delivers near-zero-touch processing for high-confidence cases, while this workbench preserves accuracy, compliance, and auditability.',
          },
        ],
      },
    ],
        ...
        <div className="fixed inset-0 z-[122] grid place-items-center">
          ...
          <Card className="relative w-[min(92vw,760px)] rounded-3xl p-6">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-2xl font-semibold text-slate-900 dark:text-slate-50">Well done!</div>
                <div className="mt-2 text-sm text-slate-600 dark:text-slate-400">
                  You now understand the Human-in-the-Loop review process. This is where human expertise combines with Agentic AI to achieve high accuracy while keeping the overall pipeline extremely fast.
