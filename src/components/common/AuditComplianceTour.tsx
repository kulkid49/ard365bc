...
type TourStep = {
  ...
  ensure?: { tab?: string; detailTab?: string }
}

const STORAGE_KEY = 'qa.auditComplianceTour.completed'
...
export function AuditComplianceTour({
  tab,
  setTab,
  detailTab,
  setDetailTab,
}: {
  tab: 'all' | 'trail' | 'gst' | 'approvals' | 'security' | 'recon'
  setTab: (v: 'all' | 'trail' | 'gst' | 'approvals' | 'security' | 'recon') => void
  detailTab: 'timeline' | 'd365' | 'gst' | 'approvals'
  setDetailTab: (v: 'timeline' | 'd365' | 'gst' | 'approvals') => void
}) {
  const [searchParams, setSearchParams] = useSearchParams()
  const forceStart = searchParams.get('tour') === 'audit'
        ...
        subSteps: [
          {
            ...
            body:
              'Welcome to the Audit Trail & Compliance Center — the single source of immutable truth for every transaction in the Agentic AR system. This page fulfills all GST, internal governance, and statutory audit requirements.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            ...
            body:
              'These tabs provide focused views: full transaction list, chronological audit log, GST-specific controls, approval history, retention policies, and D365 BC reconciliation.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            ...
            title: 'Main Transaction Table',
            body:
              'Searchable, filterable table of every processed case with Case ID, D365 Invoice #, IRN, Value, and Compliance Status. Every row links to the complete immutable audit trail.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'col-case',
            selector: '[data-tour="audit-col-caseid"]',
            title: 'Case ID',
            body:
              'Case ID is your primary trace key. It links every agent decision, every D365 action, and every human intervention into a single immutable chain-of-custody.',
          },
          {
            id: 'col-d365',
            selector: '[data-tour="audit-col-d365"]',
            title: 'D365 Invoice #',
            body:
              'D365 Invoice # is a direct deep link into Business Central. This is how audit evidence stays grounded to the real ERP record for reconciliation and statutory review.',
          },
          {
            id: 'col-irn',
            selector: '[data-tour="audit-col-irn"]',
            title: 'IRN',
            body:
              'IRN is the legally required identifier from the GST portal. It proves successful IRP registration and connects to the signed QR compliance artifact.',
          },
          {
            id: 'col-status',
            selector: '[data-tour="audit-col-status"]',
            title: 'Compliance Status',
            body:
              'Compliance Status is calculated automatically from agent outcomes, D365 API responses, and IRP acknowledgements. It flags any missing evidence before audits do.',
          },
          {
            id: 'col-event',
            selector: '[data-tour="audit-col-event"]',
            title: 'Last Event',
            body:
              'Last Event summarizes the most recent action in the Agentic flow — an agent decision, an API call, or a HITL action — so you can understand the current compliance posture instantly.',
          },
          {
            id: 'col-ts',
            selector: '[data-tour="audit-col-ts"]',
            title: 'Timestamp',
            body:
              'Every event is time-stamped for forensic accuracy and SLA enforcement. This is critical for approvals, GST filings, and post-facto audit reconciliation.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'detail',
            selector: '[data-tour="audit-detail"]',
            title: 'Detailed Audit Record View',
            body:
              'Clicking any row opens the full immutable record view. It consolidates every event across the 10-step flow — from Intake Agent to final dispatch — into one audit-ready package.',
          },
        ],
      },
      {
        id: 'timeline',
        ensure: { detailTab: 'timeline' },
        subSteps: [
          {
            id: 'timeline',
            selector: '[data-tour="audit-timeline"]',
            title: 'Vertical Audit Timeline',
            body:
              'Complete chronological timeline of all 10 steps with exact timestamps, responsible agent/user, and action type. Every automated step and Human-in-the-Loop intervention is recorded.',
          },
        ],
      },
      {
        id: 'd365',
        ensure: { detailTab: 'd365' },
        subSteps: [
          {
            id: 'd365',
            selector: '[data-tour="audit-d365log"]',
            title: 'D365 Integration Log',
            body:
              'Full log of every OData call to Dynamics 365 Business Central (Customer creation, Sales Order posting, Invoice generation) with request/response JSON and status codes.',
          },
        ],
      },
      {
        id: 'gst',
        ensure: { detailTab: 'gst' },
        subSteps: [
          {
            id: 'gst',
            selector: '[data-tour="audit-gst"]',
            title: 'GST Compliance Evidence',
            body:
              'All GST-specific evidence is stored here — IRP status, IRN linkage, and compliance artifacts — ensuring regulatory readiness and faster statutory audits.',
          },
        ],
      },
      {
        id: 'approvals',
        ensure: { detailTab: 'approvals' },
        subSteps: [
          {
            id: 'approvals',
            selector: '[data-tour="audit-approvals"]',
            title: 'Approval Chain & Digital Signatures',
            body:
              'Complete approval history with digital signatures, justifications, and escalation steps. This ties structured approvals (HITL) into the immutable compliance backbone.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            id: 'export',
            selector: '[data-tour="audit-export-controls"]',
            title: 'Export & Reconciliation Controls',
            body:
              'One-click export of full audit packages for auditors or statutory GST filing. The reconciliation engine compares every Agentic record against live D365 BC data.',
          },
        ],
      },
      {
        ...
        subSteps: [
          {
            ...
            body:
              'You now have end-to-end traceability: agent decisions, D365 API calls, IRP responses, and human approvals — all captured immutably. This reduces audit effort while strengthening compliance and governance.',
          },
        ],
      },
    ],
  ...
  useEffect(() => {
    ...
    const ensureDetail = currentStep?.ensure?.detailTab
    if (ensureDetail && detailTab !== ensureDetail) setDetailTab(ensureDetail as typeof detailTab)
  }, [celebration, currentSub?.id, currentStep?.ensure?.detailTab, currentStep?.ensure?.tab, detailTab, open, setDetailTab, setTab, tab])
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="mt-2 text-sm text-slate-600 dark:text-slate-400">
                  Excellent! You now understand how the Agentic AI system delivers complete, immutable audit trails and full GST compliance automatically. Every transaction is traceable from document intake to
                  dispatch with zero gaps. Restart this tour anytime from the help menu.
                </div>
              </div>
